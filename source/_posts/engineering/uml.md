...
![](https://s1.ax1x.com/2020/03/29/GVBGHf.png)
...
![从现实世界到业务模型](https://s1.ax1x.com/2020/03/29/GV35uQ.png)
...
![从业务模型到概念模型](https://s1.ax1x.com/2020/03/29/GV80P0.png)
...
![从概念模型到设计模型](https://s1.ax1x.com/2020/03/29/GV8RaR.png)

　　如果把三个模型的建立过程综合起来，如下图所示：

![](https://s1.ax1x.com/2020/03/29/GVYhin.png)
...
![概念层类图](https://s1.ax1x.com/2020/03/29/GZLXIf.png)

#### 说明层类图

​		说明层的观点认为，在这个层次的类图考察的是类的接口而不是实现，类图中表达的类和类关系应当是对问题领域在接口层次抽象的描述。

![说明层类图](https://s1.ax1x.com/2020/03/29/GZOAoV.png)

#### 实现层类图

​		实现层观点认为，类是实现代码的描述，类图中的类直接映射到可执行代码。

![实现层类图](https://s1.ax1x.com/2020/03/29/GZO3o6.png)

#### 常用元素参考

![类图常用元素](https://s1.ax1x.com/2020/03/30/Ge9Xod.png)

### 包图

​		在实际项目中，建模过程中获得的元素是非常多的，如果要将这些元素的关系都绘制出来，将如同蜘蛛网一样难以辨别。通过包这个容器来从大到小、从粗到细地建立关系是一种很好的办法。

![网上购物-领域包图](https://s1.ax1x.com/2020/03/29/GZO0eI.png)

​		在UML所有视图中，包图或许是最自由，约束最小的一种。除了特定的版型之外，包几乎可以用在任何阶段。

![层次包图](https://s1.ax1x.com/2020/03/29/GZO7fU.png)
...
![登机手续用例场景活动图示例](https://s1.ax1x.com/2020/03/29/GZXi1e.png)

#### 对象活动图

​		对象活动图用于展示对象的交互。

![对象活动图](https://s1.ax1x.com/2020/03/29/GZXw3F.png)

#### 泳道

​		上面的活动图描述了业务流程中活动的执行顺序，却没有描述出谁来执行这些活动，即执行业务流程的职责被遗漏了。泳道技术的引入多多少少解决了活动图不能描述对象职责的遗憾。

![对象交互泳道图](https://s1.ax1x.com/2020/03/29/GZXOC8.png)

> ​		状态图是很有用的技术，尤其在描述单个复杂对象的行为时非常有助于我们理解一个对象的行为。但是在建模时并不需要对每个对象都绘制状态图。建议，仅对领域模型中最为关键的业务对象，尤其是当其在一个或多个用例场景中参与了多个活动时，才对其建模。

#### 常用元素参考

[![活动图常用元素](https://s1.ax1x.com/2020/03/30/Ge9gLF.png)](https://imgchr.com/i/Ge9gLF)

### 状态图

​		状态图显示一个状态机，通常使用状态图来说明业务角色或业务实体可能的状态——导致状态转换的事件和状态转换引起的操作。状态机主要用于描述对象的状态变化以确定何种行为改变了对象状态，以及对象状态变化对系统的影响。

![图书生命周期状态图](https://s1.ax1x.com/2020/03/29/GZjkCT.png)

#### 常用元素参考

![状态图常用元素](https://s1.ax1x.com/2020/03/30/Ge9wIs.png)
...
![网上购买商品业务模型时序图](https://s1.ax1x.com/2020/03/29/GZjYKH.png)

#### 概念模型时序图

​		概念用例时序图通常是依据业务模型场景图来绘制的，它将业务模型场景用分析类重新绘制一遍，这样，既保留了实际业务需求，又得到了计算机实现的基本理念。

![购买商品概念模型时序图片断](https://s1.ax1x.com/2020/03/29/GZjdat.png)

#### 设计模型时序图

​		设计模型时序图使用设计类作为对象绘制。目标是实现概念模型中的某个事件流，一般以一个完整交互为单位，消息细致到方法级别。

![登录和查询事件流设计模型时序图片断](https://s1.ax1x.com/2020/03/29/GZjXIx.png)

> ​		时序图的三种应用场合是在建模过程中经常使用的动态视图。除了这些场合，在任何时候需要表达对象间的交互时，或者想分析对象的职责和接口时都可以使用时序图。

#### 常用元素参考

![时序图常用元素](https://s1.ax1x.com/2020/03/30/GepX80.png)
...
![业务模型协作图](https://s1.ax1x.com/2020/03/29/GZvFeA.png)
...
![购买商品概念模型协作图片断](https://s1.ax1x.com/2020/03/29/GZvnSS.png)

#### 设计模型协作图

​		与时序图相同，设计模型协作图使用设计类为对象来绘制。目标是实现概念模型中的某个事件流，一般以一个完整交互为单位，消息细致到方法级别。

![登录和查询事件流设计模型协作图片断](https://s1.ax1x.com/2020/03/29/GZvKyQ.png)

#### 常用元素参考

![协作图常用元素](https://s1.ax1x.com/2020/03/30/Gep9ht.png)
